...
- This standalone app repository lives at
  `/Volumes/SSD/Developer/SongWorkbench`. Older catalog notes below refer
  to the previous wrapper workspace, not to files expected in this repo. The checkout
  moved from `~/Developer/SongWorkbench` to the SSD on 2026-09-17; paths in
  `tasks/todo.md` and `tasks/lessons.md` entries predating that still name the old root.
...
- The native macOS app repo root is `/Volumes/SSD/Developer/SongWorkbench`.
...
- `/Volumes/SSD/Developer/SongWorkbench` is now its own Git repository.
...
- 2026-07-05 late: BASS DISPLAY TRANSPOSE — the chart's Transpose stepper shifted
  ...
  iPad worktree: /Volumes/SSD/Developer/SongWorkbench-ipad branch
