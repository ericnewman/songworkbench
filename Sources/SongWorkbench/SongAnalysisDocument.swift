...
struct SongAnalysisDocument: Codable, Equatable, Sendable {
    ...
    //     additive; was not bumped on its own because the schema version is part of the raw
    //     transcription cache key, and bumping it would re-transcribe every song for a
    //     chart-only field. The 13 below now carries it.
