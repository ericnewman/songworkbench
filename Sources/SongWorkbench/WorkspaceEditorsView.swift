...
struct ChordProTabEditor: View {
    ...
    private var hiddenEnergyStems: Set<StemID> {
        Set(hiddenEnergyStemsStorage.split(separator: ",").map { StemID(rawValue: String($0)) })
    }
    private func setEnergyStemHidden(_ stemID: StemID, _ hidden: Bool) {
        var stems = hiddenEnergyStems
        if hidden { stems.insert(stemID) } else { stems.remove(stemID) }
        hiddenEnergyStemsStorage = stems.map(\.rawValue).sorted().joined(separator: ",")
    }
    ...
    private var hiddenBucketStems: Set<StemID> {
        Set(hiddenBucketStemsStorage.split(separator: ",").map { StemID(rawValue: String($0)) })
    }
    private func setBucketStemHidden(_ stemID: StemID, _ hidden: Bool) {
        var stems = hiddenBucketStems
        if hidden { stems.insert(stemID) } else { stems.remove(stemID) }
    ...
    private func bucketStemMenuTitle(_ stemID: StemID) -> String {
        model.stemSet?.descriptorsByID[stemID]?.displayName
            ?? stemID.legacyKind?.displayName
            ?? stemID.rawValue
    }
                ...
                Group {
                    ...
                    case .preview:
                        ChordProAppPreview(
                            ...
                            // Only a timeline cut on the grid the chart is showing may be
                            // drawn; a stale one (metrical retune since) is withheld until
                            // "Compute Bucket Notes" recuts it.
...
struct ChordProAppPreview: View {
    ...
    /// Per-stem metronome-bucket notes, shown as one Review-only row per stem under each line.
    ...
    /// Instrument energy under a row, across the row's whole drawn span — from its left edge (the
    /// downbeat minus its pickup gutter) to `end` — so every row opens flush with audio and the
    /// instruments also show beneath the vocals (Eric, 2026-09-14). The music is continuous; an
    /// empty gutter or pickup indent read as gaps in it (Eric: "this is a continuous piece of
    /// music. There should be no gaps at the start of lines"). Clamped only at the song start.
    ///
    /// `gutterSeconds` is the ROW's own gutter, not a song constant: the energy has to start where
    /// the row's left edge actually is, or a row with no pickup would draw peaks that its ruler
    /// then clamps on top of each other at x = 0.
    private func instrumentEnergy(
        rowAnchor: TimeInterval?, rowDownbeat: TimeInterval?, gutterSeconds: TimeInterval,
        end: TimeInterval
    ) -> (series: [ChordProRowEnergySeries], start: TimeInterval, seconds: TimeInterval) {
        guard let rowAnchor else { return ([], 0, 0) }
        let start = max((rowDownbeat ?? rowAnchor) - gutterSeconds, 0)
        guard end - start > 0.05 else { return ([], start, 0) }
        let lanePeaks = instrumentLanes.map { peaks(in: (start, end), from: $0.envelope) }
        // An instrument that is essentially quiet in this row draws no line (or band) here.
        let series = InstrumentEnergyLanes.audibleLaneIndices(lanePeaks).map {
            ChordProRowEnergySeries(peaks: lanePeaks[$0], color: instrumentLanes[$0].id.laneColor)
        }
        return (series, start, end - start)
    }
    ...
    /// vocals for lyric lines; no peaks (only the window) for instrumental chord-only lines, whose
    /// energy is the row's instrument energy (`instrumentEnergy`); empty for everything else.
    private func lineStrip(
        ...
        // behind the old guitar/piano lane meant a song with no guitar/piano stem (or one not yet loaded
        ...
        return ([], max(0, window.end - window.start), window.start, .swTextSecondary)
    }
    ...
    private func blockRow(
        ...
        scale rowScale: ChordProChartScale, songGutterSeconds: TimeInterval?
    ) -> some View {
        ...
        // lines) anchor on their strip's start instead — every row
        // gets the same gutter and downbeat column, or none of the
        // left edges line up across the page.
        let rowAnchorTime =
            firstWordOnset
        ...
        // Instruments span the row from its left edge to two beats past its last word (or window)
        ...
        let energyEnd: TimeInterval = frameEnd ?? [stripEnd, tailEnd].compactMap { $0 }.max() ?? 0
    ...
    private func chordOnlyLineWindow(
        ...
        let envelopeDurations =
            [audioEnvelope?.duration].compactMap { $0 } + instrumentLanes.map(\.envelope.duration)
...
private struct ChordProPreviewBlockView: View {
    ...
    /// The playhead on EVERY row (highlight is only the active lyric line's).
    ...
    /// lane over `instrumentEnergyStart` + `instrumentEnergySeconds`.
...
/// One instrument lane's peaks under a chart row, in its lane color.
struct ChordProRowEnergySeries: Equatable {
    var peaks: [Float]
    var color: Color
}
...
private struct ChordProPreviewLineView: View {
    ...
    var songChordTimes: [TimeInterval] = []
    var highlight: ChordProLinePlaybackHighlight?
    /// The playhead on EVERY row — drives the sounding-chord amber name, onset pop/glow, and
    /// the parked amber ball. `highlight` is only non-nil on the active lyric line, which left
    /// all of those dark on instrumental rows (Eric: "the orange ball to highlight the current
    /// chord, and chord name highlighting on onset are missing").
    var playheadTime: TimeInterval?
    var beatBall: LineBeatBall?
    var beatDots: LineBeatBall?
    var ballVisibility = BouncingBallVisibility()
    ...
    /// Bucket rows with each cell at its bucket's real x. Cells are NOT collision-nudged: a
    ...
    private func chordOnsetGlow(at index: Int) -> some View {
        let intensity = chordOnsetIntensity(at: index)
        if intensity > 0 {
    ...
    /// Index of the chord SOUNDING at the playhead on THIS row, or nil: the latest onset at or
    /// before the playhead, still sounding until the next chord onset anywhere in the song
    /// (rows don't end a chord — the next chord does). Drives the amber name, the onset
    /// pop/glow, and the parked amber ball, all off `playheadTime` so they fire on every row
    /// kind (Eric: "light up the chord name in Amber at each onset, and then back to normal at
    /// the next chord onset").
    private var soundingChordIndex: Int? {
        guard let now = playheadTime, !rowChordTimes.isEmpty else { return nil }
        var best: Int?
        for (index, onset) in rowChordTimes.enumerated()
        where onset <= now && (best.map { rowChordTimes[$0] <= onset } ?? true) {
            best = index
        }
        guard let best else { return nil }
        if let next = songChordTimes.first(where: { $0 > rowChordTimes[best] + 0.001 }),
            now >= next
        {
            return nil
        }
    ...
    private func chordOnsetScale(at index: Int) -> CGFloat {
        1 + 0.3 * chordOnsetIntensity(at: index)
    }

    /// 1 exactly at a chord's onset, decaying to 0 over one beat — the attack transient the
    /// pop, glow, and ball hop all share.
    private func chordOnsetIntensity(at index: Int) -> CGFloat {
        let beat = beatLengthSeconds
        guard let now = playheadTime, rowChordTimes.indices.contains(index), beat > 0
        else { return 0 }
        let elapsed = now - rowChordTimes[index]
        guard elapsed >= 0, elapsed < beat else { return 0 }
        return CGFloat(1 - elapsed / beat)
    }

    /// The parked amber ball: sits ON the sounding chord's label and hops at each onset —
    /// current-chord indication that can never run ahead of the music, because it never
    /// travels (Eric: "The orange ball to highlight the current chord"). `xs` is the rendered
    ...
    private func soundingChordBall(chordXs xs: [CGFloat]) -> some View {
        if ballVisibility.chordPop, let index = soundingChordIndex, xs.indices.contains(index) {
            ...
            Circle()
                ...
                .opacity(0.95)
                .position(
                    x: xs[index] + labelHalfWidth,
                    y: ballTopReserve - 2 - lift * ballApexHeight)
        }
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 2) {
            ...
            if !vocalPeaks.isEmpty || !instrumentEnergy.isEmpty {
                waveformStrip
            }
        }
    }
    ...
    private var stripWidth: CGFloat {
        ...
        // A lyric row with a known window but no per-word timings still spans its real time on
        // the ruler — its strip was previously squeezed to the text width, putting the whole row
        // on the character axis.
        if rhythmicSpacing, lineDuration > 0, rhythmicWords.isEmpty {
            return max(1, rowRuler.x(atTime: rowStartTime + lineDuration))
        }
        ...
        //
        // `energyEndX` stays in: the instruments' tail is real song time beyond the last word, not
        ...
        let energyEndX =
            instrumentEnergySeconds > 0
            ? rhythmicX(forTime: instrumentEnergyStart + instrumentEnergySeconds) : 0
        return max(1, max(endX, energyEndX))
    }
    ...
    private var waveformStrip: some View {
        Canvas { context, size in
            // Any rhythmic row with a real window draws its energy at ruler positions —
            // `rhythmicX` needs no words now, so untimed lines are on the shared axis too.
            if rhythmicSpacing, lineDuration > 0 || instrumentEnergySeconds > 0 {
                func bars(
                    _ peaks: [Float], from start: TimeInterval, seconds: TimeInterval,
                    scale peakScale: Float, baseline: CGFloat? = nil, bandHeight: CGFloat? = nil
                ) -> [CGRect] {
                    let bottom = baseline ?? size.height
                    let band = bandHeight ?? size.height
                    return peaks.indices.map { index in
                        let count = Double(peaks.count)
                        let x0 = rhythmicX(forTime: start + seconds * Double(index) / count)
                        let x1 = rhythmicX(forTime: start + seconds * Double(index + 1) / count)
                        let height = max(CGFloat(peaks[index] / peakScale) * band, 1)
                        return CGRect(
                            x: x0, y: bottom - height, width: max(x1 - x0, 1), height: height)
                    }
                }
                ...
                if !instrumentEnergyOutlined, instrumentEnergySeconds > 0 {
                    for series in instrumentEnergy {
                        ...
                        {
                            context.fill(Path(rect), with: .color(series.color.opacity(0.45)))
                        }
                    }
                }
                if !vocalPeaks.isEmpty, lineDuration > 0 {
                ...
                let step = size.width / CGFloat(vocalPeaks.count)
        ...
        .frame(width: max(1, stripWidth), height: stripHeight)
    }
    ...
    private var stripHeight: CGFloat {
        ...
        guard lanes > 1 else { return scale.scaled(18) }
        return max(scale.scaled(18), lanes * scale.scaled(10) + (lanes - 1) * energyLaneGap)
    }
    ...
    /// without a usable grid. Drawn on every row KIND — a bar is every `beatsPerBar` beats in the
    /// instrumental passages too, or the page's bar columns break wherever the singing stops
    /// (Eric: "I'd expect to see a bar consistently, every 4 beats even in instrumental
    /// passages").
    private var barlineXs: [CGFloat] {
        guard showBarlines, rowDownbeatSeconds != nil, beatsPerBar > 0 else { return [] }
        let barPx = CGFloat(beatsPerBar) * pixelsPerBeat
        guard barPx > 1 else { return [] }
        // The downbeat sits at gutterPx; barlines step one bar apart across the row's content.
        let maxX = rowContentEndX
        var x = gutterPx.truncatingRemainder(dividingBy: barPx)
        if x < 0 { x += barPx }
        var result: [CGFloat] = []
        while x <= maxX + 1, result.count < 64 {
            result.append(x)
            x += barPx
        }
        return result
    ...
    private var monospaceContent: some View {
        ...
        return ZStack(alignment: .topLeading) {
            ...
            // columns continue wherever the singing stops.
            ForEach(Array(barlineXs.enumerated()), id: \.offset) { _, x in
                Rectangle()
                    .fill(Color.swTextSecondary.opacity(0.16))
                    .frame(width: 1, height: contentHeight)
    ...
    /// Frame width of a monospace-rendered row: the ruler extent when the row has a real time
    ...
    private var beatDotPositions: [CGFloat] {
        ...
        // Any rhythmic row with a real time window takes its dots from the row ruler —
        // index-space, equidistant, same axis as every other row. This must come FIRST: it
        // includes lyric rows WITHOUT per-word timings (untranscribed lines), whose dots
        // previously fell into the word-center path below (their character-interpolated
        // `wordCenters` is never empty) and rendered at a visibly different scale
        // (~52 px vs ~78 px between beats on the same page).
