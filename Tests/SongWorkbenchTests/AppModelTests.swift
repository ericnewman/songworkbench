...
final class AppModelTests: XCTestCase {
    ...
    func testStorageRootKeepsImportedCopiesOutOfTheUserLibrary() async throws {
        let url = try makeSilentWAV()
        defer { try? FileManager.default.removeItem(at: url) }
        let root = makeTestStorageRoot()
        let model = AppModel(
            store: DelayedProjectStore(document: ProjectLibraryDocument()), storageRoot: root)

        model.importSongs(from: [url])
        try await waitUntil { !model.songs.isEmpty }

        let copy = model.songs[0].url.resolvingSymlinksInPath().path
        XCTAssertTrue(
            copy.hasPrefix(root.resolvingSymlinksInPath().path),
            "imported copy landed outside the injected storage root: \(copy)")
    }
...
/// A throwaway storage root, so a test's `AppModel` never loads the user's installed models or
/// writes into their library. ponytail: left in the temp directory for the OS to clean up.
func makeTestStorageRoot() -> URL {
